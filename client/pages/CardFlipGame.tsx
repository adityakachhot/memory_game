...
interface CardFlipGameProps {
  ...
  // Multiplayer-controlled props
  controlledCards?: GameCard[];
  onRequestFlip?: (cardId: number) => void;
  isMyTurn?: boolean;
}
...
export default function CardFlipGame({
  ...
  const initializeGame = useCallback(() => {
    if (multiplayerMode) return; // deck is controlled by container in multiplayer
  ...
  // When in multiplayer, adopt controlled cards from container
  useEffect(() => {
    if (multiplayerMode && controlledCards) {
      setCards(controlledCards);
      const matched = controlledCards.filter((c) => c.isMatched).length / 2;
      setMatches(matched);
      setMoves((prev) => prev); // leave as-is
    }
  }, [multiplayerMode, controlledCards]);
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div
          ref={gameGridRef}
          className={cn(
            "grid gap-2 md:gap-4 max-w-2xl mx-auto px-2 md:px-0",
        ...
        >
          {cards.map((card) => (
            ...
            multiplayerMode && !isMyTurn && "cursor-not-allowed opacity-80",
              )}
            >
              <div
                className={cn(
              ...
              >
                ...
                <div
                  className={cn(
                    "absolute inset-0 backface-hidden rotate-y-180 rounded-lg border-2 flex items-center justify-center text-4xl md:text-6xl transition-colors",
                    card.isMatched
