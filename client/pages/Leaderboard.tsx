...
function usePVPLeaderboard() {
  ...
  useEffect(() => {
    async function run() {
      try {
        const users = collection(db, "users");
        const q = query(users, orderBy("pvpWins", "desc"), limit(20));
        const snap = await getDocs(q);
        const result: LeaderboardEntry[] = [];
        let i = 0;
        snap.forEach((d) => {
...
export default function Leaderboard() {
  ...
  const pvpRows = usePVPLeaderboard();
          ...
          <TabsContent value="pvp" className="space-y-4">
            <Card className="bg-card/50">
              <CardHeader>
                ...
                <CardDescription>Rankings based on PVP wins</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {pvpRows.map((entry) => (
                  ...
                  {pvpRows.length === 0 && (
                    <p className="text-center text-muted-foreground py-8">
                      No PVP wins yet. Challenge other players to get on the leaderboard!
                    </p>
                  )}
                </div>
              </CardContent>
