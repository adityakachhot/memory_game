...
import { ArrowLeft, RotateCcw, Trophy, Target, Zap, Play, Volume2, VolumeX } from "lucide-react";
import Layout from "@/components/Layout";
import { cn } from "@/lib/utils";

type GamePhase = "setup" | "showing" | "waiting" | "correct" | "wrong" | "gameOver";
...
export default function SimonSaysGame() {
  ...
  const colors: ColorButton[] = [
    { id: "red", name: "Red", bgColor: "bg-red-500", activeColor: "bg-red-300", sound: 330 },
    { id: "blue", name: "Blue", bgColor: "bg-blue-500", activeColor: "bg-blue-300", sound: 262 },
    { id: "green", name: "Green", bgColor: "bg-green-500", activeColor: "bg-green-300", sound: 220 },
    { id: "yellow", name: "Yellow", bgColor: "bg-yellow-500", activeColor: "bg-yellow-300", sound: 392 }
  ];
  ...
  const showSequence = useCallback((seq: ColorType[]) => {
    let index = 0;
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {gamePhase === "setup" && (
          <Card className="bg-card/50">
            <CardHeader>
              <CardTitle className="text-center">Simon Says - Color Sequence</CardTitle>
            </CardHeader>
            <CardContent className="text-center space-y-4">
              <p className="text-muted-foreground">
                Watch the sequence of colors flash, then repeat it by clicking the colors in the same order. 
                Each round adds a new color to remember!
              </p>
          ...
          <Card className="bg-card/50">
            <CardContent className="p-4">
              <div className="text-center">
                {gamePhase === "showing" && (
                  <p className="text-lg font-semibold text-primary">👁️ Watch the sequence carefully!</p>
                )}
                {gamePhase === "waiting" && (
                  <p className="text-lg font-semibold text-green-400">🎯 Repeat the sequence by clicking the colors!</p>
                )}
                {gamePhase === "correct" && (
                  <p className="text-lg font-semibold text-green-400">✅ Correct! Getting harder...</p>
                )}
                {gamePhase === "wrong" && (
                  <p className="text-lg font-semibold text-red-400">❌ Wrong sequence!</p>
                )}
                {gamePhase === "gameOver" && (
                  <div className="space-y-2">
                    <p className="text-lg font-semibold text-red-400">🎮 Game Over!</p>
                    <p className="text-sm text-muted-foreground">Final Score: {score} | Best: {bestScore}</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          {colors.map((color) => (
            <button
              ...
              className={cn(
                ...
                color.bgColor,
                activeColor === color.id ? `${color.activeColor} scale-110 shadow-2xl` : "",
                gamePhase === "wrong" && "animate-pulse",
                gamePhase === "waiting" && "hover:brightness-110 cursor-pointer",
                (gamePhase === "showing" || gamePhase === "setup") && "opacity-70"
              )}
            >
